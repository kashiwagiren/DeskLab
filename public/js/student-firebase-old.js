...
async function loadSchedule() {
  ...
  if (!room) {
    document.getElementById("scheduleDisplay").innerHTML =
      '<div class="no-schedule">Please select a room to view schedule.</div>';
    return;
  }
  ...
  try {
    const response = await fetch(
      `api/schedule.php?room=${encodeURIComponent(room)}`
    );
    const data = await response.json();
...
function displaySchedule(classes, room) {
  ...
  if (classes.length === 0) {
    scheduleDisplay.innerHTML =
      '<div class="no-schedule">No classes scheduled for this room.</div>';
    return;
  }
...
async function processStudyLoad() {
  ...
  try {
    ...
    if (data.success) {
      ...
      if (data.year_section)
        document.getElementById("yearSection").value = data.year_section;

      showNotification(
        "Success",
        "Study load processed! Please verify the information and complete your login."
      );
      showManualForm();
    } else {
      showNotification(
        "Error",
        data.error || "Failed to process study load. Please use manual entry."
      );
      showManualForm();
    }
  } catch (error) {
    showNotification(
      "Error",
      "Failed to process file. Please use manual entry."
    );
    showManualForm();
    console.error("Error:", error);
  } finally {
...
async function startCheckingApproval(requestId) {
  checkInterval = setInterval(async () => {
    try {
      const response = await fetch(
        `api/check_approval.php?request_id=${requestId}`
      );
      const data = await response.json();
...
async function startClassMonitoring(roomNumber) {
  setInterval(async () => {
    try {
      const response = await fetch(
        `api/check_upcoming_class.php?room=${encodeURIComponent(roomNumber)}`
      );
      const data = await response.json();
